...
- **Limpeza automática:** fica ligada por padrão e mantém `2 dias` (hoje e ontem). O interruptor e a janela `1|2|3|5` da página são persistidos em `feedback_notificacoes` por `feedback_notificacao`; o Python aplica a exclusão na geração seguinte. Em execução local, usar `--no-apply-cleanup` apenas para desativá-la naquela execução.
- **Participação do Rafa é importante:** menção direta, resposta a Rafa ou confirmação de que ele participa de uma conversa/atividade devem elevar a origem para pelo menos prioridade média e Review.
- **Grupos e eventos:** o resumo deve ser um parágrafo concreto sobre o que está sendo discutido, decisões e pendências. Se houver evento, encontro, aula, jogo, passeio ou atividade, preencher também `events[]` com título, data, hora, local, detalhes e URL; indicar `Não informado`/`A confirmar` quando faltar um dado, sem inventar.
