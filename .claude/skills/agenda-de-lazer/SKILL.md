...
- Este módulo nunca escreve o HTML inteiro nem faz push directo — produz o JSON canónico (`events[]`) e entrega à Rotina de Publicação (`módulo = lazer`). Os caminhos exactos de template/render/GitHub vivem em `ROTINA_PUBLICACAO.md` (mesma pasta Drive das instruções, sem sufixo de versão no nome) — confirmar sempre lá qual é a versão de template/render actualmente promovida, não assumir.
