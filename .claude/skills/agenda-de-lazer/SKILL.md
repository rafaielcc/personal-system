...
- Este módulo nunca escreve o HTML inteiro nem faz push directo — produz o JSON canónico (`events[]`) e entrega à Rotina de Publicação (`módulo = lazer`). Os caminhos exactos de template/render/GitHub/cópia Drive vivem em `ROTINA_PUBLICACAO_v1.5.md` (mesma pasta Drive das instruções) — mas atenção, esse mapa já avisa que a Agenda de Lazer teve iterações de render mais recentes que ainda não foram promovidas à tabela oficial; se o render referenciado lá parecer desactualizado face ao que encontraste na pasta `Templates/`, confirmar com o Rafa antes de escolher qual usar, não assumir.
