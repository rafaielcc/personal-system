...
- O Briefing nunca escreve o HTML inteiro nem faz push directo — produz apenas o JSON canónico (esquema na Secção 7 do documento de instruções) e entrega à Rotina de Publicação (`módulo = briefing`). Os caminhos exactos de template/render/GitHub/cópia Drive (incluindo o `weather.py` para a tab Tempo) vivem em `ROTINA_PUBLICACAO_v1.5.md` (mesma pasta Drive das instruções) e podem mudar — ir sempre lá, não assumir os valores actuais.
