...
3. Na mesma pasta Drive, procurar **`BRIEFING_DIARIO`** pelo nome (última confirmada: `BRIEFING_DIARIO_v14.0.md`) — se houver mais do que uma versão, usar sempre a de `createdTime` mais recente, não assumir que o número de versão no nome é o mais alto (o Rafa por vezes demora a apagar a versão anterior). Descarregar e seguir a rotina na íntegra — não resumir nem saltar etapas. Existe também uma cópia `_LOCAL` (`BRIEFING_DIARIO_v14.0_LOCAL.md`) que descreve a variante automatizada por Agendador de Tarefas (ver `agendas/CLAUDE.md`) — não é a que este skill activa, que é sempre a corrida interactiva completa.
...
- O Briefing nunca escreve o HTML inteiro nem faz push directo — produz apenas o JSON canónico (esquema na Secção 7 do documento de instruções) e entrega à Rotina de Publicação (`módulo = briefing`). Os caminhos exactos de template/render/GitHub vivem em `ROTINA_PUBLICACAO.md` (mesma pasta Drive das instruções, sem sufixo de versão no nome) e podem mudar — ir sempre lá, não assumir os valores actuais. Não há cópia no Drive do HTML publicado, só GitHub.
