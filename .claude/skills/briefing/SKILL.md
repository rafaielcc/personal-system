...
3. Na mesma pasta Drive, procurar **`BRIEFING_DIARIO`** pelo nome (última confirmada: `BRIEFING_DIARIO_v13.0.md`) — se houver mais do que uma versão, usar sempre a de `createdTime` mais recente, não assumir que o número de versão no nome é o mais alto (o Rafa por vezes demora a apagar a versão anterior). Descarregar e seguir a rotina na íntegra — não resumir nem saltar etapas. Existe também uma cópia `_LOCAL` (`BRIEFING_DIARIO_v13.0_LOCAL.md`) que descreve a variante automatizada por Agendador de Tarefas (ver `agendas/CLAUDE.md`) — não é a que este skill activa, que é sempre a corrida interactiva completa.
