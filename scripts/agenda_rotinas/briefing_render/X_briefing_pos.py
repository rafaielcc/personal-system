...
# -*- coding: utf-8 -*-
"""
X_briefing_pos.py — Pos-flight local do Briefing Diario (validacao + render + publicacao)
============================================================================================

O que este ficheiro e
----------------------
O ultimo passo do Briefing, depois do LLM ter interpretado o JSON do
X_briefing_pre.py e escrito o JSON canonico final. Corre so localmente
(precisa do template/render no Drive e de um checkout git de personal-system).

  1. Valida o JSON canonico final (schema, via render_v6.validate_canonical(),
     mais as regras proprias: sem campos _llm*/_preflight*, datas em sequencia).
  2. Se --pre-json for passado (deve ser sempre, no fluxo normal), cruza o final
     contra a corrida do pre: mesma data-alvo, mesmo modo, e diagnostico.fontes/
     avisos/erros identicos ao que o pre gravou — o LLM so pode mexer em
     diagnostico.notas_llm, nunca nestes campos mecanicos.
  3. Renderiza (render_v6.py + template v8, com fallback para v7) e valida o
     HTML produzido (sem blocos/tokens residuais, textos obrigatorios, tab
     Trabalho HFF so em Modo A, tab Diagnostico sempre).
  4. Copia para agendas/briefing/index.html no repo e publica via git push.

Publica sempre
---------------
Por omissao publica sem pedir nada — regra do Rafa (publicar e' o fim normal
de uma corrida do Briefing, nao um passo que precise de autorizacao). --dry-run
e' a unica forma de nao publicar, para ensaiar o render sem tocar no repo.

Uso
---
    python3 X_briefing_pos.py --json final.json --pre-json pre.json
    python3 X_briefing_pos.py --json final.json --pre-json pre.json --dry-run
"""
...
# Sem subpasta por rotina dentro de X_Rotinas_Python — mesma convencao flat que
# X_briefing_pre.py e go_X_briefing_pre.bat ja usam (pedido do Rafa).
...
REQUIRED_HTML_TEXT = ("Briefing", "Hoje", "Tarefas", "Email", "Tempo", "Rotina", "Diagnóstico")
...
def load_render_validator(render_path: Path):
    spec = importlib.util.spec_from_file_location("briefing_render_v6", render_path)
...
def validate_against_pre(final_data: dict[str, Any], pre_data: dict[str, Any], errors: list[str]) -> None:
    """Cruza o JSON canonico final (do LLM) contra o JSON do pre (a recolha mecanica) —
    nao para revalidar dados, mas para apanhar o LLM a desviar-se do que devia: correr
    contra a corrida errada, ou tocar em diagnostico.fontes/avisos/erros, que sao
    mecanicos e nunca deviam ter sido reescritos (so notas_llm e' dele)."""
    pre_modo = pre_data.get("modo") if isinstance(pre_data.get("modo"), dict) else {}
    final_meta = final_data.get("meta") if isinstance(final_data.get("meta"), dict) else {}

    pre_date = pre_modo.get("data_alvo")
    if pre_date and final_meta.get("date") != pre_date:
        errors.append(
            f"meta.date ({final_meta.get('date')!r}) nao bate com o pre (modo.data_alvo={pre_date!r}) "
            "— o LLM esta a trabalhar em cima da corrida errada do pre."
        )
    pre_mode = pre_modo.get("valor")
    if pre_mode and final_meta.get("mode") != pre_mode:
        errors.append(
            f"meta.mode ({final_meta.get('mode')!r}) nao bate com o pre (modo.valor={pre_mode!r})."
        )

    pre_fontes = pre_data.get("fontes")
    final_diag = final_data.get("diagnostico") if isinstance(final_data.get("diagnostico"), dict) else {}
    if isinstance(pre_fontes, dict) and final_diag.get("fontes") != pre_fontes:
        errors.append(
            "diagnostico.fontes no JSON final nao e' identico ao 'fontes' do pre — este campo e' "
            "mecanico, o LLM nunca deve reescreve-lo (so pode acrescentar a notas_llm)."
        )
    pre_avisos = pre_data.get("avisos")
    if isinstance(pre_avisos, list) and final_diag.get("avisos") != pre_avisos:
        errors.append("diagnostico.avisos no JSON final nao e' identico aos avisos do pre.")
    pre_erros = pre_data.get("erros")
    if isinstance(pre_erros, list) and final_diag.get("erros") != pre_erros:
        errors.append("diagnostico.erros no JSON final nao e' identico aos erros do pre.")
...
def validate_html(path: Path, mode: str) -> list[str]:
    ...
    block_pattern = (
        r"<!--/?(?:HFF_BTN|HFF_PANEL|TASK_DAY|TASK_ITEM_T|TASK_ITEM_E|TASK_ITEM_X|"
        r"CAL_CELL|CAL_DAY_PANEL|WEATHER_ROW|WINDOW_ROW|EMAIL_SECTION:[a-z_]+|"
        r"DIAG_FONTE_ROW|DIAG_SECTION:[a-z_]+|DIAG_ROW:[a-z_]+)-->"
    )
...
def postflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    pre_json = Path(args.pre_json) if args.pre_json else None
    ...
    if pre_json is not None and not pre_json.exists():
        errors.append(f"pre_json em falta: {pre_json}")
    if errors:
    ...
    if pre_json is None:
        warnings.append(
            "--pre-json nao foi passado: sem cruzamento contra a recolha do pre "
            "(usar so para testes manuais do JSON final isolado)."
        )
    ...
    try:
        ...
        if pre_json is not None:
            ...
            if isinstance(pre_data, dict) and isinstance(data, dict):
                validate_against_pre(data, pre_data, errors)
            else:
                errors.append("pre_json ou json final nao sao objectos — nao foi possivel cruzar.")
        if errors:
            result["status"] = "blocked"
            return result
        ...
        if errors:
            result["status"] = "blocked"
            return result
        # Publica sempre — regra do Rafa, nunca pedir autorizacao. --dry-run e' a unica
        # forma de nao publicar, e e' para testes/ensaios, nao para o fluxo normal.
        if not args.dry_run:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pos (validacao + render + publicacao) da rotina Briefing.")
    ...
    parser.add_argument("--render", help="Render script do Briefing. Default: render_v6.py.")
    parser.add_argument("--html", help="Destino HTML gerado. Default: _work timestamp local.")
    parser.add_argument("--manifest", default=str(ROUTINE_DIR / "briefing_pos_manifest.json"), help="Manifesto desta corrida.")
    parser.add_argument("--repo", help="Checkout personal-system para copia/publicacao.")
    parser.add_argument("--dry-run", action="store_true",
                        help="Renderiza e valida mas NAO copia nem publica. Por omissao publica sempre — regra do Rafa, nunca pede autorizacao.")
