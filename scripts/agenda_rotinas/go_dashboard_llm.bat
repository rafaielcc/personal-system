...
REM  (04:15, seg-sex, apos go_X_briefing_pre.bat). O
...
REM       partir do _briefing.json ja produzido pela 1a tarefa (04:00) e
...
REM     go_dashboard_llm.bat
REM
REM  Uso agendado (Agendador de Tarefas do Windows, 04:15, seg-sex --
...
set "POS_SCRIPT=%AGENDA%\X_Rotinas_Python\X_dashboard_pos.py"
...
REM O Python local faz o render/push do Dashboard; o LLM so produz o JSON canonico.
set "PROMPT=%PROMPT% REGRA LOCAL PRIORITARIA: no passo 5, le G:/My Drive/Claude_PRJ/Rafa_profile.md e G:/My Drive/Claude_PRJ/Feedback_log.json por caminho local, grava somente o JSON final em X_Outputs com o mesmo timestamp do preflight, e NAO executa render.py, Git, Drive MCP ou publicacao para o Dashboard. O BAT chama X_dashboard_pos.py logo a seguir e verifica o resultado. Se o pre_filtro.cirped.needs_action for false, nao le a rotina CIRPED."
...
echo.
python "%POS_SCRIPT%"
set "POS_RC=%ERRORLEVEL%"
if not "%POS_RC%"=="0" (
    echo [ERRO] Dashboard nao publicado. Codigo de saida: %POS_RC%
    exit /b %POS_RC%
)
if not "%RC%"=="0" (
    echo [ATENCAO] Dashboard publicado, mas o LLM reportou erro em outra etapa: %RC%
    exit /b %RC%
)
echo [OK] Dashboard publicado e passo LLM concluido.
exit /b 0
